...
class Recommender:
    ...
    def __init__(self, name, prometheus_connection, endpoint, grafana_dashboard_uid, start_date, end_date, tolerance, batch_size ) -> None:
        ...
        self._namespace_filter = self.__get_namespace_filter()

    def recommend(self):
        self.process_metric_data()

    def __get_namespace_filter(self):
    ...
    def get_namespace_from_labels(self, label_filter_prom):
        ...
        result = self.prometheus_connection.custom_query(label_query)
        for data in result:
    ...
    def get_metric_data(self,query,batch_ids, epoch: int = None):
        ...
        start = timer()
        metric_data = self.prometheus_connection.custom_query(**parameters)
        end = timer()
        self.endpointLogger.info("finished running query: %s" , str(parameters["query"]))
        if epoch:
    ...
    def get_grouped_aggregation(self,df_data_list, agg_func: str = "max"):
        return concat(df_data_list).groupby(self.key).agg({"value": agg_func, "dummy_counter": "sum"}).reset_index()

    def get_merged_data(self,df_data1, df_data2):
        return df_data1.merge(df_data2, on=self.key, how="left")
    
    def get_request_data_batch(self,end_date, cluster_ids_filter):
        resource_req_query = self.requestQuery.replace(
    ...
    def get_usage_data_batch(self,start_date, end_date, cluster_ids_filter):
        cpu_usage_max = []
        memory_usage_max = []
        cpu_usage_query = self.cpuMaxQuery.replace(
            "NAMESPACE_FILTER", self._namespace_filter
        )
        mem_usage_query = self.memoryMaxQuery.replace(
            "NAMESPACE_FILTER", self._namespace_filter
        )
    ...
    def process_metric_data(self):
        MainLogger.info("Working on endpoint %s " , self.endpoint) 
        cluster_ids_batch = self.get_cluster_id_batch(self.end_date, self.batch_size)
        df_request_data = self.get_request_data_batch(self.end_date, cluster_ids_batch)
        (
            df_cpu_usage_max,
            df_memory_usage_max
        ) = self.get_usage_data_batch(self.start_date, self.end_date, cluster_ids_batch)
        ...
        df_merged["cpu_recommendation_request"] = df_merged.cpu_usage_max * (1 + self.tolerance / 100)
        df_merged["memory_recommendation_request"] = df_merged.memory_usage_max * (1 + self.tolerance / 100)
